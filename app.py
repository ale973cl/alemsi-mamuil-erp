...
def render_casino():
    ...
    roles_casino = ["Cocina", "Bodega", "Finanzas"]
    if usuario and usuario.get("rol") in roles_casino:
        ...
        if rol == "Cocina":
                    ...
                    if st.button("▶️ INICIAR JORNADA", type="primary", use_container_width=True, disabled=not confirmar):
                        with conn.session as ses:
                            ...
                            for _,r in df_prod.iterrows():
                                execute_sql(ses, "INSERT INTO jornada_detalle (fecha,servicio,tipo_opcion,plato,reservadas,producidas,entregadas) VALUES (%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (fecha,servicio,plato) DO UPDATE SET reservadas=EXCLUDED.reservadas", (fecha_iso,str(r['servicio']),str(r['tipo_opcion']),str(r['plato']),int(r['reservadas']),int(r['reservadas']),0))
                            ses.commit()
            ...
            if df.empty:
                ...
                st.caption("El recordatorio automático de las 17:00 y la carga por enlace seguro quedan documentados para la siguiente capa de automatización; no dependen de que Streamlit permanezca abierto.")
    else:
        ...
        with st.form("login_casino"):
            ...
            if st.form_submit_button("Ingresar",type="primary",use_container_width=True):
                conn=get_conn()
                df=conn.query("SELECT username,rol,nombre,COALESCE(activo,1) AS activo FROM usuarios WHERE username=:username AND pwd=:pwd",params={"username":u,"pwd":hash_pwd(p)},ttl=0)
                if not df.empty and int(df.iloc[0]['activo'])==1 and df.iloc[0]['rol'] in roles_casino:
                    st.session_state.usuario={"username":df.iloc[0]['username'],"rol":df.iloc[0]['rol'],"nombre":df.iloc[0]['nombre']}; st.session_state.portal_actual="casino"; st.rerun()
                else: st.error("Usuario no válido, deshabilitado o sin permiso para Personal de Casino.")

def render_admin():
    if st.session_state.usuario and st.session_state.usuario["rol"] in ["AdminTotal","AdminCasino","Gerencia"]:
        ...
        with tab_usuarios:
            if st.session_state.usuario.get("rol") != "AdminTotal":
                st.info("Solo el Administrador Total puede crear usuarios, cambiar roles o activar/desactivar funciones.")
            else:
                st.markdown("#### 👥 Gestión de usuarios y permisos")
                conn=get_conn()
                dfu=conn.query("SELECT username,nombre,rol,COALESCE(activo,1) AS activo,fecha_creacion FROM usuarios ORDER BY username",ttl=0)
                ...
                with st.form("crear_usuario_total"):
                    ...
                    with c2:
                        nr=st.selectbox("Rol*",["Cocina","Bodega","Finanzas","Gerencia","AdminCasino","AdminTotal"])
                    ...
                    if st.form_submit_button("Crear / actualizar usuario",type="primary",use_container_width=True):
                        if nu.strip() and nn.strip() and np:
                            with conn.session as ses:
                                execute_sql(ses,"INSERT INTO usuarios (username,pwd,rol,nombre,activo,fecha_creacion) VALUES (%s,%s,%s,%s,1,%s) ON CONFLICT (username) DO UPDATE SET pwd=EXCLUDED.pwd,rol=EXCLUDED.rol,nombre=EXCLUDED.nombre,activo=1",(nu.strip(),hash_pwd(np),nr,nn.strip(),datetime.now().isoformat()))
                                ses.commit()
                ...
                if not dfu.empty:
                    ...
                    rol_n=st.selectbox("Rol",["Cocina","Bodega","Finanzas","Gerencia","AdminCasino","AdminTotal"],index=["Cocina","Bodega","Finanzas","Gerencia","AdminCasino","AdminTotal"].index(str(rowu['rol'])) if str(rowu['rol']) in ["Cocina","Bodega","Finanzas","Gerencia","AdminCasino","AdminTotal"] else 0)
        ...
        with st.form("login_admin"):
            ...
            if st.form_submit_button("Ingresar", type="primary", use_container_width=True):
                conn=get_conn()
                df=conn.query("SELECT username,rol,nombre,COALESCE(activo,1) AS activo FROM usuarios WHERE username=:username AND pwd=:pwd", params={"username": u, "pwd": hash_pwd(p)}, ttl=0)
                if not df.empty and int(df.iloc[0]['activo'])==1 and df.iloc[0]['rol'] in ["AdminTotal","AdminCasino","Gerencia"]:
...
if st.session_state.usuario:
    ...
    st.session_state.portal_actual = "administracion" if rol_activo in ["AdminTotal", "AdminCasino", "Gerencia"] else "casino"
