        ...
        with tab_reserva:
            if not st.session_state.dias_sel:
                ...
                if idx<len(dias):
                    ...
                    # Copia defensiva: no modificar la constante MINUTA al agregar datos de BD.
                    minuta_dia = {servicio: list(platos) for servicio, platos in MINUTA.get(dnom, {}).items()}
                    # Intentar cargar de DB minutas activas
                    try:
                        conn=get_conn()
                        df_min_db = conn.query("SELECT servicio, plato FROM minutas WHERE dia_semana=:dia_semana AND activo=1", params={"dia_semana": dnom}, ttl=0)
            ...
            with st.form("reclamo"):
                ...
                if st.form_submit_button("Enviar", type="primary", use_container_width=True):
                    if mensaje:
                        conn=get_conn()
                        with conn.session as s:
                            execute_sql(s, "INSERT INTO reclamos_sugerencias (rut,nombre,tipo,categoria,mensaje,fecha,estado) VALUES (%s,%s,%s,%s,%s,%s,%s)", (rut,nombre,tipo,categoria,mensaje, datetime.now().isoformat(),"Pendiente"))
                            s.commit()
                        st.success("Enviado")
    else:
