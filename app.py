...
def render_casino():
    ...
    if usuario and usuario.get("rol") in ["Cocina", "Bodega", "Finanzas"]:
        ...
        if rol == "Cocina":
            tab_resumen, tab_minuta, tab_recetas, tab_bodega = st.tabs([
                "🍽️ Resumen diario", "📅 Minuta del día", "📖 Recetas", "📦 Bodega operativa"
            ])

            with tab_resumen:
                st.markdown("#### Producción diaria consolidada")
                fecha_cocina = st.date_input("Fecha de producción", value=date.today(), key="fecha_cocina")
                fecha_iso = fecha_cocina.isoformat()
                conn = get_conn()
                df = conn.query(
                    """
                ...
                total_dia = int(df["cantidad"].sum()) if not df.empty else 0
                st.metric("Porciones reservadas del día", total_dia)
                if df.empty:
                    st.info("No hay reservas que contabilizar para esta fecha.")
                else:
                    for servicio in orden:
                        grupo = df[df["servicio"].astype(str) == servicio]
                        if grupo.empty:
                            continue
                        total_servicio = int(grupo["cantidad"].sum())
                        st.markdown(f"### {servicio} · {total_servicio} porciones")
                        vista = grupo[["plato", "cantidad"]].rename(columns={"plato":"Plato", "cantidad":"Cantidad"})
                        st.dataframe(vista, use_container_width=True, hide_index=True)

            with tab_minuta:
                st.markdown("#### Minuta correspondiente al día")
                fecha_minuta = st.date_input("Selecciona el día", value=st.session_state.get("fecha_cocina", date.today()), key="fecha_minuta_cocina")
            ...
            with tab_recetas:
                ...
                conn = get_conn()
                dfr = conn.query("SELECT plato, insumo, cantidad, unidad, instrucciones FROM recetas ORDER BY plato, insumo", ttl=0)
                if dfr.empty:
                    st.info("Aún no hay recetas configuradas.")
                else:
                    st.dataframe(dfr, use_container_width=True, hide_index=True)
                st.caption("En esta candidata las recetas son de consulta. El descuento de Bodega al iniciar producción queda desactivado hasta completar las pruebas.")

            with tab_bodega:
                st.markdown("#### Bodega operativa · consulta para Cocina")
                conn = get_conn()
                dfb = conn.query("SELECT codigo_insumo, nombre_articulo, unidad, stock, critico, caduca, seccion FROM bodega_inventario ORDER BY nombre_articulo LIMIT 300", ttl=0)
                st.dataframe(dfb, use_container_width=True, hide_index=True)
                st.caption("Cocina puede consultar existencias. Las cargas autorizadas y el descuento al iniciar producción se habilitarán en una etapa posterior con trazabilidad.")
