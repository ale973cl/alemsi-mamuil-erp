        ...
        with tab_reserva:
            ...
            if resultado_anterior:
                ...
                if resultado_anterior.get("vouchers"):
                    st.info(f"Códigos: {', '.join(resultado_anterior['vouchers'])}")
            ...
            if not st.session_state.dias_sel:
                ...
                with st.form("calendario_reserva_v21", clear_on_submit=False):
                        ...
                        for columna, dia in zip(columnas, semana):
                            with columna:
                                ...
                                if not pertenece_mes:
                                    ...
                                    marcado = st.checkbox(
                                        str(dia.day),
                                        key=f"cal_dia_{dia.isoformat()}",
                                        help=dia.strftime("%d/%m/%Y"),
                                    )
                                    if marcado:
                                        seleccion.append(dia.isoformat())
                                else:
                ...
                if not st.session_state.get("reserva_revisar", False):
                                ...
                                for pos_serv, servicio in enumerate(servicios):
                                    ...
                                    with columnas[pos_serv % 2]:
                                        elegido = st.selectbox(
                                            ...
                                            key=f"menu_v21_{f_iso}_{servicio}",
                                        )
                                    if elegido:
                    ...
                    if confirmar:
                                ...
                                if es_alemsi:
                                    for f_iso, dnom, servicio, plato, declaracion in detalle:
                                        ...
                                        execute_sql(
                                            ...
                                            "(rut,fecha,servicio,plato,plato_reservado,codigo,precio,precio_aplicado,institucion,correo,metodo_pago,estado_pago,estado_consumo,fecha_creacion,tipo_registro) "
                                            "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                                            (rut, f_iso, servicio, plato, plato, None, 0, 0, institucion, correo_cli, "Interno ALEMSI", "No aplica", estado_bd, datetime.now().isoformat(), "CONSUMO_INTERNO"),
                                        )
                                else:
                                    precio_por_linea = {}
                                    for f_iso in dias:
                                    ...
                                    for f_iso, dnom, servicio, plato, precio_ref in detalle:
                                        ...
                                        execute_sql(
                                            ...
                                            "(rut,fecha,servicio,plato,plato_reservado,codigo,precio,precio_aplicado,institucion,correo,metodo_pago,estado_pago,estado_consumo,fecha_creacion,tipo_registro) "
                                            "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                                            (rut, f_iso, servicio, plato, plato, codigo, precio_ref, precio_por_linea[(f_iso, servicio, plato)], institucion, correo_cli, metodo, "Pendiente", "Pendiente", datetime.now().isoformat(), "RESERVA_COMERCIAL"),
                                        )
                        ...
                        if es_alemsi:
                            ...
                            mensaje_resultado = "Declaración ALEMSI guardada correctamente. No se generó comprobante ni correo."
                            ok_resultado = True
                        else:
                            ...
                            correo_ok, correo_msg = enviar_email(correo_cli, f"Comprobante de reserva Mamuil Malal · {len(dias)} día(s)", html_comprobante)
                            ...
                            mensaje_resultado = f"Reserva guardada y comprobante enviado a {correo_cli}." if correo_ok else f"Reserva guardada, pero el comprobante no pudo enviarse: {correo_msg}"
                            if not cocina_ok:
                        ...
                        st.session_state.pedidos = {}
                        st.session_state.reserva_revisar = False
                        st.session_state.resultado_reserva = {"ok": ok_resultado, "mensaje": mensaje_resultado, "vouchers": vouchers}
                        st.rerun()
...
with t_admin:
    if st.session_state.usuario and st.session_state.usuario["rol"] in ["Admin","Gerencia"]:
        ...
        tab_reportes, tab_g, tab_prod, tab_minuta, tab_exc, tab_inst, tab_correos = st.tabs(["📊 REPORTES FINANCIEROS Y COCINA (NUEVO)","📈 Gerencia","📦 Productos","🍽️ Minutas","⚖️ Excepciones","🏢 Instituciones","📧 Correos"])
