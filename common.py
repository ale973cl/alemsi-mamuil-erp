...
def get_precio_institucion(institucion: str):
    try:
        conn=get_conn()
        df = conn.query("SELECT precio_dia, precio_especial, regla_activa FROM instituciones WHERE nombre=:institucion", params={"institucion": institucion}, ttl=0)
        if not df.empty:
...
def get_precio_persona_institucion(rut: str, institucion: str):
    try:
        conn=get_conn()
        df = conn.query("SELECT precio_especial, descripcion FROM excepciones_personas WHERE rut=:rut AND activa=1", params={"rut": normalizar_rut_db(rut)}, ttl=0)
        if not df.empty:
...
def get_precio(plato, servicio):
    try:
        conn=get_conn()
        df = conn.query("SELECT valor FROM platos WHERE nombre=:plato", params={"plato": plato}, ttl=0)
        if not df.empty:
