...
def init_db():
    ...
    try:
        with conn.session as s:
            ...
            usuarios_default = [
                ("admin","admin123","AdminTotal","Administrador Total"),
            ...
            for u,p,r,n in usuarios_default:
                execute_sql(s, "INSERT INTO usuarios (username,pwd,rol,nombre,activo,fecha_creacion,debe_cambiar_password) VALUES (%s,%s,%s,%s,1,%s,1) ON CONFLICT (username) DO UPDATE SET rol=EXCLUDED.rol,nombre=EXCLUDED.nombre,activo=1", (u, hash_pwd(p), r, n, datetime.now().isoformat()))
            # Bodega ya no es un portal independiente: el acceso queda integrado a Cocina/Administración.
            execute_sql(s, "UPDATE usuarios SET activo=0 WHERE username='bodega' AND rol='Bodega'")
            # Compatibilidad: el usuario histórico 'admin' pasa a Administrador Total sin cambiar su clave.
            execute_sql(s, "UPDATE usuarios SET rol='AdminTotal', nombre='Administrador Total', activo=1 WHERE username='admin' AND rol IN ('Admin','AdminTotal')")
            # Migración única v2.1.3.5: habilita usuarios de prueba con claves temporales conocidas.
            # El marcador evita que init_db vuelva a sobrescribir contraseñas después del primer despliegue.
            bootstrap = execute_sql(s, "SELECT clave FROM migraciones_app WHERE clave=%s", ("BOOTSTRAP_USUARIOS_V2135",)).first()
            if not bootstrap:
                usuarios_temporales = [
                    ("admin_casino", "casino123", "AdminCasino", "Administrador Casino"),
                    ("operaciones", "operaciones123", "Operaciones", "Operaciones"),
                    ("cocina", "cocina123", "Cocina", "Cocina"),
                    ("finanzas", "finanzas123", "Finanzas", "Finanzas"),
                    ("gerencia", "gerencia123", "Gerencia", "Gerencia"),
                ]
                for u,pwd,rol,nombre in usuarios_temporales:
                    execute_sql(s, "UPDATE usuarios SET pwd=%s,rol=%s,nombre=%s,activo=1,debe_cambiar_password=1 WHERE username=%s", (hash_pwd(pwd), rol, nombre, u))
                execute_sql(s, "UPDATE usuarios SET activo=0 WHERE username='bodega'")
                execute_sql(s, "INSERT INTO migraciones_app (clave,aplicado_at) VALUES (%s,%s)", ("BOOTSTRAP_USUARIOS_V2135", datetime.now().isoformat()))

            # Estructura inicial simplificada; los perfiles adicionales se crean desde Admin Total.
            execute_sql(s, "UPDATE usuarios SET activo=0 WHERE username IN ('admin_casino','operaciones','bodega','finanzas@alemsi.cl')")
