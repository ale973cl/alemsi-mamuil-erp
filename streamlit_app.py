    ...
    if st.session_state.rut_actual:
        ...
        with tab_reserva:
            if not st.session_state.dias_sel:
                ...
                dias=[primer_dia_mes+timedelta(days=i) for i in range(ultimo_dia) if primer_dia_mes+timedelta(days=i) >= hoy]
                st.caption(f"Mostrando días disponibles de {hoy.strftime('%B %Y')} - Precio día {formato_clp(precio_dia)}")
                cols=st.columns(4)
                ...
                # Recupera sesiones antiguas o incompletas sin perder los días elegidos.
                if not isinstance(st.session_state.pedidos, dict):
                ...
                if idx<len(dias):
                    ...
                    # Primero carga la minuta real por fecha desde Supabase.
                    # Solo usa la minuta histórica por día de semana como respaldo.
                    minuta_dia = {}
                    tipos_opcion = {}
                    try:
                        conn=get_conn()
                        df_min_db = conn.query(
                            "SELECT servicio, tipo_opcion, plato FROM minutas WHERE fecha=:fecha AND activo=1 ORDER BY servicio, CASE tipo_opcion WHEN 'Opción 1' THEN 1 WHEN 'Opción 2' THEN 2 ELSE 3 END",
                            params={"fecha": f_iso},
                            ttl=300,
                        )
                        if not df_min_db.empty:
                            for _, r in df_min_db.iterrows():
                                serv=str(r['servicio']); plato=str(r['plato'])
                                minuta_dia.setdefault(serv, []).append(plato)
                                tipos_opcion[(serv, plato)] = str(r.get('tipo_opcion') or '')
                        else:
                            minuta_dia = {servicio: list(platos) for servicio, platos in MINUTA.get(dnom, {}).items()}
                    except Exception:
                        minuta_dia = {servicio: list(platos) for servicio, platos in MINUTA.get(dnom, {}).items()}

                    tiene_uno=False
                    if not minuta_dia:
                        st.warning("No hay minuta configurada para este día. Vuelve atrás y elige otra fecha, o solicita a Administración que cargue la minuta.")
                    for serv, platos in minuta_dia.items():
                        ...
                        for j, plato in enumerate(platos):
                            with cols[j%2]:
                                ...
                                etiqueta_tipo = tipos_opcion.get((serv, plato), "")
                                texto_boton = f"{etiqueta_tipo}: {plato}" if etiqueta_tipo else plato
                                if st.button(f"{texto_boton} - {formato_clp(get_precio(plato,serv))}", key=f"{f_iso}_{serv}_{plato}", use_container_width=True, type="primary" if is_sel else "secondary"):
                                    st.session_state.pedidos.setdefault(f_iso, {})[serv] = plato
                                    st.rerun()
                    ...
                    if st.button("✅ CONFIRMAR Y ENVIAR COMPROBANTE POR CORREO", type="primary", use_container_width=True):
                        conn=get_conn()
                        # Distribuye el precio diario entre las líneas del día para que
                        # los reportes sumen exactamente el total mostrado al comensal.
                        precio_por_linea = {}
                        for f_iso in dias:
                            lineas_dia = [x for x in detalle if x[0] == f_iso]
                            base, resto = divmod(int(precio_dia), len(lineas_dia))
                            for pos, linea in enumerate(lineas_dia):
                                precio_por_linea[(linea[0], linea[2], linea[3])] = base + (1 if pos < resto else 0)

                        with conn.session as s:
                            vouchers=[]
                            dfc=conn.query("SELECT correo FROM comensales WHERE rut=:rut", params={"rut": rut}, ttl=0)
                            ...
                            for f_iso,dnom,serv,plato,pr in detalle:
                                ...
                                execute_sql(
                                    ...
                                    (rut,f_iso,serv,plato,plato, cod, pr, precio_por_linea[(f_iso, serv, plato)], institucion, correo_cli, metodo, "Pendiente", "Pendiente", datetime.now().isoformat())
                                )
                                vouchers.append(cod)
                            s.commit()
                        # Descontar bodega
                        ...
                        <div style="font-family:Arial, sans-serif; padding:24px; border:2px solid #0A2F6B; border-radius:16px; max-width:700px;">
                            ...
                            <div style="padding:16px 0;">
                                ...
                                <p style="background:#F8F9FA; padding:12px; border-radius:8px; font-size:13px;">
                                    <b>Minuta:</b> Cada día se asignó según el día de la semana ({", ".join([f"{['Lunes','Martes','Miércoles','Jueves','Viernes','Sábado','Domingo'][date.fromisoformat(f).weekday()]} {date.fromisoformat(f).strftime('%d/%m')}" for f in dias])})<br>
