        ...
        if rol == "Coordinacion":
            ...
            if vista_finanzas == "🧾 Validar comprobantes":
                ...
                with accion_col:
                    ...
                    # VALIDAR/OBSERVAR/RECHAZAR ya no dispara un rerun completo ni vuelve
                    ...
                    if confirmar_revision:
                        if requiere_obs_val and not obs_val.strip():
                            st.error("Debes ingresar una observación para observar o rechazar el comprobante.")
                            return
                        estado_pago_destino = {
                            "VALIDADO": "Pagado",
                            "OBSERVADO": "Observado",
            ...
            if vista_finanzas == "🏢 Por institución":
                ...
                with st.form(f"fin_form_reserva_{ref_sel}", clear_on_submit=False):
                    nuevo_comp = st.selectbox(
                        "Resultado de validación", estados_comp,
                    ...
                    obs_comp = st.text_area(
                        "Observación", value=str(comp.get("observacion_validacion") or ""),
                        key=f"fin_obs_reserva_{ref_sel}",
                    )
                    requiere_obs = nuevo_comp in ["OBSERVADO","RECHAZADO"]
                    guardar_validacion = st.form_submit_button(
                        "Guardar validación de la reserva", type="primary", use_container_width=True,
                        disabled=((requiere_obs and not obs_comp.strip()) or not archivo_disponible),
                    )

                if guardar_validacion:
                    if requiere_obs and not obs_comp.strip():
                        st.error("Debes ingresar una observación para observar o rechazar el comprobante.")
                        return
                    estado_pago_destino = {
                    ...
                    _refrescar_finanzas("✅ Reserva y comprobante actualizados en conjunto.", "success")
                return
