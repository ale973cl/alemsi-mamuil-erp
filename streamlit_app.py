...
<h1>🍽️ Mamuil Malal</h1>
<p>Portal de Servicios de Alimentación · ALEMSI</p>
</div>
            ...
            with st.form("reclamo"):
                ...
                if st.form_submit_button("Enviar", type="primary", use_container_width=True):
                    if mensaje:
                        conn=get_conn()
                        with conn.session as s:
                            exec_sql(s, "INSERT INTO reclamos_sugerencias (rut,nombre,tipo,categoria,mensaje,fecha,estado) VALUES (%s,%s,%s,%s,%s,%s,%s)", (rut,nombre,tipo,categoria,mensaje, datetime.now().isoformat(),"Pendiente"))
                            s.commit()
                        st.success("Enviado")
    else:
        st.markdown("### Reserva de alimentación")
        st.caption("Ingresa tu RUT para reservar.")
        with st.expander("🧪 Acceso de pruebas", expanded=False):
            st.write("Perfil demo: **12.345.678-5 · Comensal Demo**")
            if st.button("Ingresar con perfil demo", key="btn_demo_comensal", use_container_width=True):
                st.session_state.rut_actual="12345678-5"
                st.rerun()
                    ...
                    with st.form("reg"):
                        ...
                        if st.form_submit_button("Registrarme", type="primary", use_container_width=True):
                            if nombre and tel and correo and institucion:
                                conn=get_conn()
                                with conn.session as s:
                                    exec_sql(s, "INSERT INTO comensales (rut,nombre,telefono,correo,institucion,fecha_registro) VALUES (%s,%s,%s,%s,%s,%s) ON CONFLICT (rut) DO UPDATE SET nombre=%s, telefono=%s, correo=%s, institucion=%s", (rn,nombre,tel,correo,institucion, datetime.now().isoformat(), nombre, tel, correo, institucion))
                                    s.commit()
                                st.session_state.rut_actual=rn; st.rerun()
...
if main_view == "🏢 ADMINISTRACIÓN":
    if st.session_state.usuario and st.session_state.usuario["rol"] in ["Admin","Gerencia"]:
        ...
        admin_view = st.radio(
            ...
            ["📊 Reportes", "📈 Gerencia", "📦 Productos", "🍽️ Minutas", "⚖️ Excepciones", "🏢 Instituciones", "🧪 Pruebas"],
            key="admin_view",
            horizontal=True,
            label_visibility="collapsed",
        )
            ...
            with col1:
                if not df_pend.empty:
                    ...
                    if st.button("✅ Marcar como Pagado", type="primary"):
                        conn=get_conn()
                        with conn.session as s:
                            exec_sql(s, "UPDATE solicitudes SET estado_pago=%s WHERE codigo=%s", ("Pagado", id_pago))
                            s.commit()
            ...
            with st.form("add_minuta"):
                ...
                if st.form_submit_button("Agregar a minuta", type="primary", use_container_width=True):
                    if plato:
                        conn=get_conn()
                        with conn.session as s:
                            exec_sql(s, "INSERT INTO minutas (dia_semana,servicio,plato,activo) VALUES (%s,%s,%s,1)", (dia,serv,plato))
                            exec_sql(s, "INSERT INTO platos (nombre,servicio,valor,activo) VALUES (%s,%s,%s,1) ON CONFLICT DO NOTHING", (plato,serv,6500))
                            s.commit()
                ...
                if st.form_submit_button("💾 Guardar precios especiales", type="primary", use_container_width=True):
                    ...
                    with conn.session as s:
                        for nombre, marcado in seleccionadas.items():
                            if marcado:
                                exec_sql(s, "UPDATE instituciones SET precio_especial=%s, regla_activa=1, descripcion=%s WHERE nombre=%s", (precio_especial_global, motivo_global, nombre))
                            else:
                                exec_sql(s, "UPDATE instituciones SET regla_activa=0 WHERE nombre=%s", (nombre,))
                        s.commit()
        ...
        if admin_view == "🧪 Pruebas":
            st.markdown("### 🧪 Entorno de pruebas")
            st.info("Estas herramientas afectan únicamente los registros de reserva y el perfil demo. No borran comensales reales, usuarios, minutas, inventario ni reclamos.")
            conn=get_conn()

            st.markdown("#### Perfil de pruebas")
            st.code("RUT: 12.345.678-5\nNombre: Comensal Demo\nCorreo: ale973@gmail.com\nInstitución: Visitas")
            if st.button("Crear o restaurar perfil demo", type="primary", use_container_width=True):
                with conn.session as s:
                    exec_sql(s, """
                    ...
                    """, ("12345678-5", "Comensal Demo", "+56900000000", "ale973@gmail.com", "Visitas", datetime.now().isoformat()))
                    s.commit()
                st.success("Perfil demo disponible. Puedes ingresar desde Comensal → Acceso de pruebas.")

            st.divider()
            st.markdown("#### Limpiar reservas")
            cantidad = conn.query("SELECT COUNT(*) AS total FROM solicitudes", ttl=0)
            total_registros = int(cantidad.iloc[0]["total"]) if not cantidad.empty else 0
            st.metric("Registros de reserva actuales", total_registros)
            confirmacion = st.text_input("Escribe BORRAR RESERVAS para confirmar", key="confirmar_borrado_reservas")
            if st.button("Borrar todos los datos de reserva", disabled=confirmacion != "BORRAR RESERVAS", use_container_width=True):
                with conn.session as s:
                    exec_sql(s, "DELETE FROM solicitudes")
                    s.commit()
                st.success("Se eliminaron los datos de reserva. Los demás módulos y datos maestros se conservaron.")
                st.rerun()

            st.divider()
            st.markdown("#### Preparar prueba desde cero")
            st.caption("Elimina reservas y deja creado el perfil demo en una sola operación.")
            confirmacion_total = st.text_input("Escribe PREPARAR PRUEBA", key="confirmar_preparar_prueba")
            if st.button("Preparar entorno limpio", disabled=confirmacion_total != "PREPARAR PRUEBA", type="primary", use_container_width=True):
                with conn.session as s:
                    exec_sql(s, "DELETE FROM solicitudes")
                    exec_sql(s, """
                        INSERT INTO comensales (rut,nombre,telefono,correo,institucion,fecha_registro)
                    ...
                    """, ("12345678-5", "Comensal Demo", "+56900000000", "ale973@gmail.com", "Visitas", datetime.now().isoformat()))
                    s.commit()
                st.success("Entorno de prueba preparado correctamente.")
                st.rerun()
