    ...
    if st.session_state.usuario and st.session_state.usuario["rol"] in ["AdminTotal","AdminCasino","Operaciones","Gerencia"]:
        ...
        if modulo_admin == "👥 Usuarios":
            ...
            if not dfu.empty:
                ...
                st.dataframe(vista_usuarios[["username","nombre","rol","Estado","Cambio contraseña","fecha_creacion"]].rename(columns={"username":"Usuario","nombre":"Nombre","rol":"Perfil","fecha_creacion":"Fecha de creación"}), use_container_width=True, hide_index=True)
            ...
            with st.expander("➕ Crear usuario nuevo", expanded=dfu.empty):
                ...
                if crear:
                    if not nu.strip() or not nn.strip() or len(np) < 8:
                        st.error("Completa usuario y nombre. La contraseña temporal debe tener al menos 8 caracteres.")
                        ...
                        with conn.session as ses:
                            execute_sql(ses,"INSERT INTO usuarios (username,pwd,rol,nombre,activo,fecha_creacion,debe_cambiar_password) VALUES (%s,%s,%s,%s,1,%s,1)",(nu.strip(),hash_pwd(np),nr,nn.strip(),datetime.now().isoformat()))
                            ses.commit()
...
def render_bootstrap_admin():
    ...
    if crear:
        ...
        elif not usuario.strip() or not nombre.strip() or len(clave) < 10:
            st.error("Completa los datos. La contraseña inicial debe tener al menos 10 caracteres.")
        elif clave != repetir:
            st.error("Las contraseñas no coinciden.")
        else:
            conn = get_conn()
            with conn.session as ses:
                execute_sql(ses,"INSERT INTO usuarios (username,pwd,rol,nombre,activo,fecha_creacion,debe_cambiar_password) VALUES (%s,%s,'AdminTotal',%s,1,%s,0)",(usuario.strip(),hash_pwd(clave),nombre.strip(),datetime.now().isoformat()))
                ses.commit()
...
def render_login_personal():
    ...
    if ingresar:
        ...
        if df.empty or int(df.iloc[0]["activo"]) != 1:
            ...
            return
        fila=df.iloc[0]
        st.session_state.usuario={"username":fila["username"],"rol":fila["rol"],"nombre":fila["nombre"],"debe_cambiar_password":int(fila["debe_cambiar_password"])}
