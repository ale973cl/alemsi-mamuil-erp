...
BACKUP_TABLES = [
    ...
    "auditoria_acciones", "configuracion_correos"
]
...
def cargar_usuarios_admin():
    conn = get_conn()
    return conn.query(
        "SELECT username,nombre,rol,COALESCE(activo,1) AS activo,COALESCE(debe_cambiar_password,0) AS debe_cambiar_password,fecha_creacion FROM usuarios ORDER BY activo DESC,username",
        ttl=60,
    )
...
if token_q:
    ...
    if archivo is not None and archivo.size > 10*1024*1024:
        ...
        with conn.session as ses:
            execute_sql(ses,"INSERT INTO comprobantes_pago (referencia_reserva,pago_token,rut,nombre_archivo,mime_type,contenido,fecha_carga,estado) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",(ref,token_q,str(df_token.iloc[0]['rut']),archivo.name,archivo.type,contenido,datetime.now().isoformat(),"RECIBIDO"))
...
def render_admin():
    if st.session_state.usuario and st.session_state.usuario["rol"] in ["AdminTotal","AdminCasino","Operaciones","Gerencia"]:
        ...
        if rol_admin == "AdminTotal":
            modulos_admin += ["👥 Usuarios","🧹 Depuración","🛡️ Respaldo"]
            ...
            with st.expander("➕ Crear usuario nuevo", expanded=dfu.empty):
                ...
                if crear:
                    if not nu.strip() or not nn.strip() or "@" not in ne or len(np) < 8:
                        ...
                        limpiar_cache_usuarios(); st.success("Usuario creado. Deberá cambiar su contraseña en el primer ingreso."); st.rerun()
            ...
            if not dfu.empty:
                ...
                if guardar_usuario:
                    if selu == st.session_state.usuario.get('username') and activo_n == 0:
                        ...
                        with conn.session as ses:
                            execute_sql(ses,"UPDATE usuarios SET nombre=%s,activo=%s,rol=%s WHERE username=%s",(nombre_n.strip(),activo_n,rol_n,selu))
                        ...
                        limpiar_cache_usuarios(); st.success("Usuario y permisos actualizados."); st.rerun()
        ...
        if modulo_admin == "🛡️ Respaldo":
            ...
            if st.session_state.usuario.get('rol') != 'AdminTotal':
                ...
                if st.session_state.get("ultimo_backup_bytes"):
                    ...
                    c3.metric("Versión", meta_backup.get("version", "v2.1.3.9"))
...
def render_login_personal():
    ...
    with st.expander("🔑 ¿Olvidaste tu contraseña? Recuperar acceso"):
        st.caption("Se enviará una contraseña temporal al correo asociado a tu usuario. La contraseña anterior no se muestra ni se envía.")
        with st.form("recuperar_password_personal"):
            usuario_rec = st.text_input("Usuario", key="usuario_recuperacion")
            correo_rec = st.text_input("Correo asociado", key="correo_recuperacion")
            recuperar = st.form_submit_button("Enviar recuperación al correo", use_container_width=True)
        if recuperar:
            usuario_rec=usuario_rec.strip()
            correo_rec=correo_rec.strip().lower()
            if not usuario_rec or "@" not in correo_rec:
                st.error("Ingresa tu usuario y el correo asociado.")
            else:
                conn=get_conn()
                dfu=conn.query("SELECT username,nombre,correo,COALESCE(activo,1) AS activo FROM usuarios WHERE username=:u", params={"u":usuario_rec}, ttl=0)
                # Respuesta genérica para no revelar si un usuario existe.
                if not dfu.empty and int(dfu.iloc[0]["activo"]) == 1 and str(dfu.iloc[0].get("correo") or "").strip().lower() == correo_rec:
                    ...
                    """
                    ok,msg=enviar_email(correo_rec,"Recuperación de acceso ALEMSI",html)
                    if not ok:
                        st.error("No fue posible enviar el correo de recuperación. Tu contraseña actual no fue modificada.")
                    else:
                        try:
                            with conn.session as ses:
                                execute_sql(ses, "UPDATE usuarios SET pwd=%s,debe_cambiar_password=1 WHERE username=%s", (hash_pwd(temporal), usuario_rec))
                                ses.commit()
                            registrar_auditoria(usuario_rec,"RECUPERAR_PASSWORD","usuarios",usuario_rec,"","contraseña temporal emitida","Recuperación por correo")
                            correo_oculto = correo_rec.split("@",1)[0][:1] + "***@" + correo_rec.split("@",1)[1]
                            st.success(f"Correo de recuperación enviado correctamente a {correo_oculto}. Usa la contraseña temporal recibida para ingresar.")
                        except Exception:
                            st.error("El correo fue enviado, pero no fue posible activar la contraseña temporal. Contacta al Administrador Total antes de intentar ingresar.")
                else:
                    st.warning("No se pudo iniciar la recuperación con los datos ingresados. Verifica usuario y correo asociado.")
