    ...
    if usuario and usuario.get("rol") in roles_casino:
        ...
        if rol == "Cocina":
            ...
            vista_finanzas = st.radio(
                "Sección Finanzas",
                ["🏢 Por institución", "📎 Comprobantes", "📊 Resumen"],
                horizontal=True,
            ...
            if vista_finanzas == "📊 Resumen":
                ...
                st.bar_chart(resumen.set_index("institucion")["Monto"])
                return

            if vista_finanzas == "🏢 Por institución":
                st.markdown("#### Gestión financiera por institución")
                st.caption(
                    "Orden de revisión: Institución → Comensal → Reserva → Fechas → Servicios. "
                ...
                if str(comp.get("drive_file_id") or "").strip():
                    if st.button(
                        "Preparar comprobante desde Google Drive",
                        key=f"fin_drive_inst_{int(comp['id'])}",
                        use_container_width=True,
                    ):
                        st.session_state[f"fin_drive_inst_bytes_{int(comp['id'])}"] = (
                            descargar_comprobante_drive(str(comp["drive_file_id"]))
                        )
                    contenido_comp = st.session_state.get(
                        f"fin_drive_inst_bytes_{int(comp['id'])}"
                    )
                else:
                    # FIN-07E: el binario se carga solo para el comprobante seleccionado.
                    with conn.session as ses:
                        row_bin = ses.execute(
                            text("SELECT contenido FROM comprobantes_pago WHERE id=:id"),
                            {"id": int(comp["id"])},
                        ).fetchone()
                    if row_bin is not None and row_bin[0] is not None:
                        try:
                            contenido_comp = bytes(row_bin[0])
                        except Exception:
                            contenido_comp = row_bin[0]

                if contenido_comp:
                    st.download_button(
                        "⬇️ Descargar / visualizar comprobante",
                        contenido_comp,
                        file_name=str(comp["nombre_archivo"] or f"comprobante_{int(comp['id'])}"),
                        mime=str(comp["mime_type"] or "application/octet-stream"),
                        use_container_width=True,
                    )
                elif str(comp.get("drive_file_id") or "").strip():
                    st.caption("Pulsa “Preparar comprobante desde Google Drive” para cargarlo de forma privada.")
                ...
                if st.button(
                    ...
                    type="primary",
                    use_container_width=True,
                    disabled=(requiere_obs and not obs_comp.strip()),
...
def render_admin():
    if st.session_state.usuario and st.session_state.usuario["rol"] in ["AdminTotal","AdminCasino","Operaciones","Gerencia"]:
        ...
        if modulo_admin == "📈 Dashboard":
            ...
            st.caption(
                "Gerencia puede analizar Institución → Comensal → Reserva → Fechas → Servicios."
            )
