    ...
    if usuario and usuario.get("rol") in roles_casino:
        ...
        if rol == "Cocina":
            ...
            vista_finanzas = st.radio(
                "Sección Finanzas",
                ["📊 Gestión financiera", "📎 Comprobantes"],
                horizontal=True,
            ...
            if vista_finanzas == "📎 Comprobantes":
                st.markdown("#### 📎 Comprobantes de pago")
                st.caption("Los archivos quedan asociados a la reserva. Puedes buscar por RUT, comensal, referencia o estado.")

                df_comp = conn.query("""
                    SELECT cp.id,cp.referencia_reserva,cp.rut,
                           COALESCE(c.nombre,'') AS nombre,
                           COALESCE(c.institucion,'') AS institucion,
                           cp.nombre_archivo,cp.mime_type,cp.fecha_carga,cp.estado,
                           cp.validado_por,cp.fecha_validacion,cp.observacion_validacion,
                           cp.drive_file_id,cp.drive_url,cp.storage_provider,
                           cp.contenido,
                           MAX(s.estado_pago) AS estado_pago
                    FROM comprobantes_pago cp
                    LEFT JOIN comensales c ON c.rut=cp.rut
                    LEFT JOIN solicitudes s ON s.referencia_reserva=cp.referencia_reserva
                    GROUP BY cp.id,cp.referencia_reserva,cp.rut,c.nombre,c.institucion,
                             cp.nombre_archivo,cp.mime_type,cp.fecha_carga,cp.estado,
                             cp.validado_por,cp.fecha_validacion,cp.observacion_validacion,
                             cp.drive_file_id,cp.drive_url,cp.storage_provider,cp.contenido
                ...
                with f1:
                    buscar = st.text_input("Buscar por RUT, nombre o referencia", key="fin_comp_buscar")
                with f2:
                    estados = ["Todos"] + sorted(df_comp["estado"].fillna("RECIBIDO").astype(str).unique().tolist())
                    estado_f = st.selectbox("Estado comprobante", estados, key="fin_comp_estado")
                with f3:
                    instituciones = ["Todas"] + sorted([x for x in df_comp["institucion"].dropna().astype(str).unique().tolist() if x])
                    inst_f = st.selectbox("Institución", instituciones, key="fin_comp_inst")

                vista = df_comp.copy()
                if buscar.strip():
                    q = buscar.strip().casefold()
                    mask = (
                        vista["rut"].fillna("").astype(str).str.casefold().str.contains(q, regex=False) |
                        vista["nombre"].fillna("").astype(str).str.casefold().str.contains(q, regex=False) |
                        vista["referencia_reserva"].fillna("").astype(str).str.casefold().str.contains(q, regex=False)
                    )
                    vista = vista[mask]
                if estado_f != "Todos":
                    vista = vista[vista["estado"].fillna("RECIBIDO").astype(str) == estado_f]
                if inst_f != "Todas":
                    vista = vista[vista["institucion"].fillna("").astype(str) == inst_f]

                columnas = ["id","referencia_reserva","rut","nombre","institucion","fecha_carga","estado","estado_pago","storage_provider"]
                st.dataframe(
                    _tabla_visible(
                        vista[columnas],
                        {
                            "id":"ID","referencia_reserva":"Referencia","rut":"RUT","nombre":"Comensal",
                            "institucion":"Institución","fecha_carga":"Fecha carga","estado":"Estado comprobante",
                            "estado_pago":"Estado pago","storage_provider":"Almacenamiento"
                        },
                        ["fecha_carga"],
                    ),
                    use_container_width=True,
                ...
                comp_id = st.selectbox(
                    ...
                    vista["id"].astype(int).tolist(),
                    format_func=lambda x: (
                        f"#{x} · {vista[vista['id'].astype(int)==int(x)].iloc[0]['referencia_reserva']} · "
                        f"{vista[vista['id'].astype(int)==int(x)].iloc[0]['nombre']}"
                    ),
                    key="fin_comp_id",
                )
                comp = vista[vista["id"].astype(int)==int(comp_id)].iloc[0]

                st.markdown(
                    f"**{comp['nombre']} · {comp['rut']} · {comp['institucion']}**  \\n"
                    f"Referencia: **{comp['referencia_reserva']}**"
                )

                contenido_archivo = None
                if str(comp.get("drive_file_id") or "").strip():
                    if st.button("Preparar archivo desde Google Drive", use_container_width=True, key=f"drive_prepare_{comp_id}"):
                        with st.spinner("Leyendo comprobante desde Drive..."):
                            st.session_state[f"drive_file_{comp_id}"] = descargar_comprobante_drive(str(comp["drive_file_id"]))
                    contenido_archivo = st.session_state.get(f"drive_file_{comp_id}")
                elif comp.get("contenido") is not None:
                    try:
                        contenido_archivo = bytes(comp["contenido"])
                    except Exception:
                        contenido_archivo = comp["contenido"]

                if contenido_archivo:
                    st.download_button(
                        "⬇️ Descargar / visualizar comprobante",
                        contenido_archivo,
                        file_name=str(comp["nombre_archivo"] or f"comprobante_{comp_id}"),
                        mime=str(comp["mime_type"] or "application/octet-stream"),
                        use_container_width=True,
                    )
                elif str(comp.get("drive_file_id") or "").strip():
                    st.caption("Pulsa “Preparar archivo desde Google Drive” para descargarlo de forma privada.")
                else:
                    st.warning("El registro no contiene un archivo recuperable.")

                estado_actual = str(comp.get("estado") or "RECIBIDO")
                estados_gestion = ["RECIBIDO","VALIDADO","OBSERVADO","RECHAZADO"]
                nuevo_estado = st.selectbox(
                    "Resultado de revisión",
                    estados_gestion,
                    index=estados_gestion.index(estado_actual) if estado_actual in estados_gestion else 0,
                    key=f"fin_estado_comp_{comp_id}",
                )
                observacion = st.text_area(
                    "Observación",
                    value=str(comp.get("observacion_validacion") or ""),
                    key=f"fin_obs_comp_{comp_id}",
                )
                requiere_obs = nuevo_estado in ["OBSERVADO","RECHAZADO"]
                if st.button(
                    "Guardar validación del comprobante",
                    type="primary",
                    use_container_width=True,
                    disabled=(requiere_obs and not observacion.strip()),
                    key=f"guardar_comp_{comp_id}",
                ):
                    mapa_pago = {
                    ...
                    with conn.session as ses:
                        execute_sql(
                            ses,
                            "UPDATE comprobantes_pago SET estado=%s,validado_por=%s,fecha_validacion=%s,observacion_validacion=%s WHERE id=%s",
                            (
                                nuevo_estado,
                        ...
                        execute_sql(
                            ses,
                            "UPDATE solicitudes SET estado_pago=%s,motivo_estado_pago=%s WHERE referencia_reserva=%s",
                            (
                                mapa_pago[nuevo_estado],
                                observacion.strip() or None,
                                str(comp["referencia_reserva"]),
                            ),
                        )
                        ses.commit()
                    registrar_auditoria(
            ...
            if df.empty:
                ...
                df_comp=conn.query("SELECT id,nombre_archivo,mime_type,fecha_carga,estado,contenido,drive_file_id,drive_url,storage_provider FROM comprobantes_pago WHERE referencia_reserva=:r ORDER BY fecha_carga DESC",params={"r":ref},ttl=0)
                if not df_comp.empty:
                    st.success(f"Comprobante recibido · {df_comp.iloc[0]['fecha_carga']}")
                    comp=df_comp.iloc[0]
                    contenido_comp = None
                    if str(comp.get('drive_file_id') or '').strip():
                        if st.button("Preparar comprobante desde Drive", key=f"prep_fin_{ref}", use_container_width=True):
                            st.session_state[f"fin_archivo_{ref}"] = descargar_comprobante_drive(str(comp['drive_file_id']))
                        contenido_comp = st.session_state.get(f"fin_archivo_{ref}")
                    elif comp.get('contenido') is not None:
                        try: contenido_comp = bytes(comp['contenido'])
                        except Exception: contenido_comp = comp['contenido']
                    if contenido_comp:
                        st.download_button("📎 Descargar comprobante para revisión",contenido_comp,file_name=str(comp['nombre_archivo']),mime=str(comp['mime_type'] or 'application/octet-stream'),use_container_width=True)
                else:
                ...
                if st.button("Guardar gestión financiera",type="primary",use_container_width=True,disabled=(requiere_motivo and not motivo.strip())):
                    with conn.session as ses:
                        ...
                        estado_comp = "VALIDADO" if estado_nuevo == "Pagado" else ("OBSERVADO" if estado_nuevo == "Observado" else ("RECIBIDO" if estado_nuevo == "Comprobante recibido" else "PENDIENTE"))
                        execute_sql(ses,"UPDATE comprobantes_pago SET estado=%s,validado_por=%s,fecha_validacion=%s,observacion_validacion=%s WHERE referencia_reserva=%s",(estado_comp,usuario.get('username') if estado_nuevo in ['Pagado','Observado'] else None,datetime.now().isoformat() if estado_nuevo in ['Pagado','Observado'] else None,motivo or None,ref))
...
def render_admin():
    if st.session_state.usuario and st.session_state.usuario["rol"] in ["AdminTotal","AdminCasino","Operaciones","Gerencia"]:
        ...
        modulos_admin = ["📊 Reportes","📋 Planilla reservas","📈 Dashboard","📦 Inventario / Bodega","🍽️ Minutas","⚖️ Excepciones","🏢 Instituciones","💳 Modalidades de pago","📧 Correos"]
        ...
        if modulo_admin == "📋 Planilla reservas":
        ...
        if modulo_admin == "📦 Inventario / Bodega":
        ...
        if modulo_admin == "🏦 Datos transferencia":
            ...
            with st.form("config_banco_alemsi"):
                c1,c2 = st.columns(2)
                with c1:
                    titular_b = st.text_input("Titular / Razón Social*", value=actual.get("titular",""))
                    rut_b = st.text_input("RUT*", value=actual.get("rut",""))
                    banco_b = st.text_input("Banco*", value=actual.get("banco",""))
                with c2:
                    tipo_b = st.text_input("Tipo de cuenta*", value=actual.get("tipo_cuenta",""))
                    numero_b = st.text_input("N° de cuenta*", value=actual.get("numero_cuenta",""))
                    correo_b = st.text_input("Correo para comprobantes*", value=actual.get("correo_comprobantes",""))
        ...
        if modulo_admin == "👥 Usuarios":
            ...
            with st.expander("➕ Crear usuario nuevo", expanded=dfu.empty):
                ...
                if crear:
                    if not nu.strip() or not nn.strip() or "@" not in ne or len(np) < 8:
                        st.error("Completa usuario, nombre y correo válido. La contraseña temporal debe tener al menos 8 caracteres.")
                        ...
                        with conn.session as ses:
                            execute_sql(ses,"INSERT INTO usuarios (username,pwd,rol,nombre,correo,activo,fecha_creacion,debe_cambiar_password) VALUES (%s,%s,%s,%s,%s,1,%s,1)",(nu.strip(),hash_pwd(np),nr,nn.strip(),ne.strip().lower(),datetime.now().isoformat()))
                            ses.commit()
                        registrar_auditoria(st.session_state.usuario.get('username'),'CREAR_USUARIO','usuarios',nu.strip(),'',nr,'')
                        st.success("Usuario creado. Deberá cambiar su contraseña en el primer ingreso."); refrescar_vista_persistente(limpiar_cache_usuarios)
            ...
            if not dfu.empty:
                ...
                with st.expander("🔑 Restablecer contraseña temporal"):
                    ...
                    if do_reset:
                        if len(temp) < 8 or not motivo_reset.strip():
                            ...
                            registrar_auditoria(st.session_state.usuario.get('username'),'RESET_PASSWORD','usuarios',selu,'','hash temporal',motivo_reset)
                            limpiar_cache_usuarios(); st.success("Contraseña restablecida. El usuario deberá cambiarla en el próximo ingreso.")
        ...
        if modulo_admin == "🛡️ Respaldo":
            ...
            if st.session_state.usuario.get('rol') != 'AdminTotal':
                ...
                if st.session_state.get("ultimo_backup_bytes"):
                    ...
                    c3.metric("Versión", meta_backup.get("version", "v2.1.3.13"))
