...
def _render_minuta_semanal(df_minuta, fecha_base, titulo=True):
    """MIN-VIS-03: calendario semanal común Lunes-Domingo para perfiles autorizados."""
    if titulo:
        lunes, domingo = _semana_lunes_domingo(fecha_base)
        st.markdown(f"#### Semana {lunes.strftime('%d/%m/%Y')} → {domingo.strftime('%d/%m/%Y')}")
    if df_minuta is None or df_minuta.empty:
        st.info("No hay minuta cargada para esta semana.")
        return
    datos = df_minuta.copy()
    datos["fecha_dt"] = pd.to_datetime(datos["fecha"], errors="coerce").dt.date
    lunes, _ = _semana_lunes_domingo(fecha_base)
    ...
    for idx, col in enumerate(cols):
        ...
        with col:
            st.markdown(f"**{nombres[idx]}**  \n{dia.strftime('%d/%m/%Y')}")
            gd = datos[datos["fecha_dt"] == dia]
            if gd.empty:
                st.caption("Sin minuta")
                continue
            for servicio in ["Desayuno","Almuerzo","Once","Cena"]:
                gs = gd[gd["servicio"].astype(str) == servicio]
                if gs.empty:
                    continue
                st.markdown(f"**{servicio}**")
                for _, rr in gs.iterrows():
                    op = opcion_lbl.get(str(rr.get("tipo_opcion","")), str(rr.get("tipo_opcion","")).title())
                    st.caption(f"{op}: {rr.get('plato','—')}")
    ...
    if st.session_state.usuario and st.session_state.usuario["rol"] in ["AdminTotal","AdminCasino","Operaciones","Gerencia"]:
        ...
        if modulo_admin == "🍽️ Minutas":
            ...
            if df_min.empty:
                ...
                df_min=df_min.copy(); df_min['fecha_dt']=pd.to_datetime(df_min['fecha']).dt.date
                ...
                while semana<=fin_cal:
                    cols_semana=st.columns(7,gap="small")
                    for idx in range(7):
                        dia=semana+timedelta(days=idx)
                        with cols_semana[idx]:
                            if dia.month!=mes_ref.month:
                                st.markdown(f"**{nombres_dia[idx]}**\n\n—")
                                continue
                            st.markdown(f"**{nombres_dia[idx]} {dia.strftime('%d/%m')}**")
                            gd=df_min[df_min['fecha_dt']==dia]
                            if gd.empty:
                                st.caption("Sin minuta")
                                continue
                            lineas=[]
                            for serv in ["Desayuno","Almuerzo","Once","Cena"]:
                                gs=gd[gd['servicio'].astype(str)==serv]
                                if gs.empty: continue
                                platos=[]
                                for _,rr in gs.iterrows():
                                    op=opcion_lbl.get(str(rr['tipo_opcion']),str(rr['tipo_opcion']).title())
                                    platos.append(f"{op}: {rr['plato']}")
                                lineas.append(f"**{serv}**\n"+"\n".join(platos))
                            st.markdown("\n\n".join(lineas))
                    st.divider()
